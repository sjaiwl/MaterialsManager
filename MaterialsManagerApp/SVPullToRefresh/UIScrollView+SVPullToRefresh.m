                                 ...
                                 position:(SVPullToRefreshPosition)position
{
    ...
    if (!self.pullToRefreshView) {
        ...
        SVPullToRefreshView *view = [[SVPullToRefreshView alloc]
            initWithFrame:CGRectMake(
                              0, yOrigin, self.bounds.size.width, SVPullToRefreshViewHeight)];
...
- (void)setPullToRefreshView:(SVPullToRefreshView *)pullToRefreshView
{
    ...
    objc_setAssociatedObject(
        self, &UIScrollViewPullToRefreshView, pullToRefreshView, OBJC_ASSOCIATION_ASSIGN);
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh
{
    ...
    if (!showsPullToRefresh) {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            self.pullToRefreshView.frame
                = CGRectMake(0, yOrigin, self.bounds.size.width, SVPullToRefreshViewHeight);
        }
    }
}
...
// public properties
@synthesize pullToRefreshActionHandler, arrowColor, textColor, activityIndicatorViewColor,
    activityIndicatorViewStyle, lastUpdatedDate, dateFormatter;
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(@"Pull down to sync...", ),
                                      NSLocalizedString(@"Release to sync...", ),
                                      NSLocalizedString(@"Syncing...", ), nil];
...
- (void)layoutSubviews
{
    ...
    if (hasCustomView) {
        ...
        [customView
            setFrame:CGRectMake(origin.x, origin.y, viewBounds.size.width, viewBounds.size.height)];
    }
    else {
        switch (self.state) {
        ...
        CGFloat leftViewWidth
            = MAX(self.arrow.bounds.size.width, self.activityIndicatorView.bounds.size.width);
        ...
        CGFloat labelX = (self.bounds.size.width / 2) - (totalMaxWidth / 2) + leftViewWidth
            + margin;
        ...
        if (subtitleSize.height > 0) {
            ...
            self.titleLabel.frame
                = CGRectIntegral(CGRectMake(labelX, titleY, titleSize.width, titleSize.height));
            self.subtitleLabel.frame = CGRectIntegral(CGRectMake(labelX,
                titleY + titleSize.height + marginY, subtitleSize.width, subtitleSize.height));
        }
        else {
        ...
        switch (self.position) {
            ...
            case SVPullToRefreshPositionBottom:
                scrollOffsetThreshold
                    = MAX(self.scrollView.contentSize.height - self.scrollView.bounds.size.height,
                          0.0f) + self.bounds.size.height + self.originalBottomInset;
                break;
        }
            ...
            case SVPullToRefreshPositionBottom:
                if (self.scrollView.contentSize.height >= self.scrollView.bounds.size.height) {
                    offset = MAX(self.scrollView.contentSize.height
                            - self.scrollView.bounds.size.height + self.bounds.size.height,
                        0.0f);
...
{
    if (!_arrow) {
        _arrow = [[SVPullToRefreshArrow alloc]
...
- (UILabel *)titleLabel
{
    if (!_titleLabel) {
        ...
        _titleLabel.text = NSLocalizedString(@"Pull down to sync...", );
...
- (void)setLastUpdatedDate:(NSDate *)newLastUpdatedDate
{
    ...
    self.dateLabel.text =
        [NSString stringWithFormat:NSLocalizedString(@"Last updated: %@", ), newLastUpdatedDate
                      ? [self.dateFormatter stringFromDate:newLastUpdatedDate]
                      : NSLocalizedString(@"Never Synced", )];
}
...
- (void)setDateFormatter:(NSDateFormatter *)newDateFormatter
{
    ...
    self.dateLabel.text =
        [NSString stringWithFormat:NSLocalizedString(@"Last updated: %@", ), self.lastUpdatedDate
                      ? [newDateFormatter stringFromDate:self.lastUpdatedDate]
                      : NSLocalizedString(@"Never Synced", )];
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 5) {
        NSArray *alphaGradientColors =
            [NSArray arrayWithObjects:(id)[self.arrowColor colorWithAlphaComponent:0].CGColor,
                     (id)[self.arrowColor colorWithAlphaComponent:1].CGColor, nil];
        alphaGradient = CGGradientCreateWithColors(
            colorSpace, (__bridge CFArrayRef)alphaGradientColors, alphaGradientLocations);
    }
    else {
        ...
        colors[7] = 1;
        alphaGradient
            = CGGradientCreateWithColorComponents(colorSpace, colors, alphaGradientLocations, 2);
    }
