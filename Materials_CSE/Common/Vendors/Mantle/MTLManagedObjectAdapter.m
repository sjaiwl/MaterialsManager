...
@property (nonatomic, copy, readonly) NSSet *propertyKeysToExcludeModel2ManagedObject;
...
- (id)modelFromManagedObject:(NSManagedObject *)managedObject
            processedObjects:(CFMutableDictionaryRef)processedObjects
                       error:(NSError **)error;
...
- (id)initWithModelClass:(Class)modelClass
{
    ...
    if ([modelClass respondsToSelector:@selector(relationshipModelClassesByPropertyKey)]) {
        _relationshipModelClassesByPropertyKey =
            [[modelClass relationshipModelClassesByPropertyKey] copy];
    }
    if ([modelClass respondsToSelector:@selector(propertyKeysToExcludeModel2ManagedObject)]) {
        _propertyKeysToExcludeModel2ManagedObject =
            [[modelClass propertyKeysToExcludeModel2ManagedObject] copy];
    }
                       ...
                       error:(NSError **)error
{
    ...
    BOOL (^setValueForKey)(NSString *, id) = ^(NSString *key, id value) {
        ...
        if (![model validateValue:&replaceableValue forKey:key error:error])
            return NO;
    ...
    for (NSString *propertyKey in [self.modelClass propertyKeys]) {
        ...
        BOOL (^deserializeAttribute)(NSAttributeDescription *)
            = ^(NSAttributeDescription *attributeDescription) {
                id value = performInContext(context, ^{
                    return [managedObject valueForKey:managedObjectKey];
                });

                NSValueTransformer *attributeTransformer =
                    [self entityAttributeTransformerForKey:propertyKey];
                if (attributeTransformer != nil)
                    value = [attributeTransformer reverseTransformedValue:value];

                return setValueForKey(propertyKey, value);
            };

        BOOL (^deserializeRelationship)(
            NSRelationshipDescription *) = ^(NSRelationshipDescription *relationshipDescription) {
            Class nestedClass = self.relationshipModelClassesByPropertyKey[propertyKey];
            if (nestedClass == nil) {
                [NSException raise:NSInvalidArgumentException
                            format:@"No class specified for decoding relationship at key \"%@\" in "
                            @"managed object %@",
                            managedObjectKey, managedObject];
            }
            ...
            if ([relationshipDescription isToMany]) {
                id models = performInContext(context, ^id {
                    ...
                    NSMutableArray *models =
                        [NSMutableArray arrayWithCapacity:[relationshipCollection count]];

                    for (NSManagedObject *nestedObject in relationshipCollection) {
                        MTLModel *model = [self.class modelOfClass:nestedClass
                                                 fromManagedObject:nestedObject
        ...
        BOOL (^deserializeProperty)(
            NSPropertyDescription *) = ^(NSPropertyDescription *propertyDescription) {
            if (propertyDescription == nil) {
                if (error != NULL) {
                    NSString *failureReason = [NSString
                        stringWithFormat:NSLocalizedString(
                                             @"No property by name \"%@\" exists on the entity.",
                                             @""),
                        managedObjectKey];

                    NSDictionary *userInfo = @{
                        NSLocalizedDescriptionKey :
                            NSLocalizedString(@"Could not deserialize managed object", @""),
                        NSLocalizedFailureReasonErrorKey : failureReason,
                    };

                    *error =
                        [NSError errorWithDomain:MTLManagedObjectAdapterErrorDomain
                                            code:MTLManagedObjectAdapterErrorInvalidManagedObjectKey
                                        userInfo:userInfo];
                }
            ...
            if ([propertyClassName isEqual:@"NSAttributeDescription"]) {
                ...
                if (error != NULL) {
                    NSString *failureReason = [NSString
                        stringWithFormat:NSLocalizedString(
                                             @"Property descriptions of class %@ are unsupported.",
                                             @""),
                        propertyClassName];
                    ...
                    *error = [NSError
                        errorWithDomain:MTLManagedObjectAdapterErrorDomain
                                   code:
                                       MTLManagedObjectAdapterErrorUnsupportedManagedObjectPropertyType
                               userInfo:userInfo];
                }
             ...
             error:(NSError **)error
{
    ...
    for (NSString *mappedPropertyKey in [modelClass managedObjectKeysByPropertyKey]) {
        ...
        if (error != NULL) {
            NSDictionary *userInfo = @{
                NSLocalizedDescriptionKey :
                    NSLocalizedString(@"Invalid entity attribute mapping", nil),
                NSLocalizedFailureReasonErrorKey : [NSString
                    stringWithFormat:NSLocalizedString(@"%1$@ could not be parsed because its "
                                         @"entity attribute mapping contains "
                                         @"illegal property keys.",
                                         nil),
                    modelClass]
            };
            ...
            *error =
                [NSError errorWithDomain:MTLManagedObjectAdapterErrorDomain
                                    code:MTLManagedObjectAdapterErrorInvalidManagedObjectMapping
                                userInfo:userInfo];
        }
    ...
    CFMutableDictionaryRef processedObjects = CFDictionaryCreateMutable(
             ...
             error:(NSError **)error
{
    ...
    const void *existingModel
        = CFDictionaryGetValue(processedObjects, (__bridge void *)managedObject);
            ...
            if (error != NULL) {
                NSDictionary *userInfo = @{
                    ...
                    NSLocalizedFailureReasonErrorKey : NSLocalizedString(
                        @"No model class could be found to deserialize the object.", @"")
                };

                *error = [NSError errorWithDomain:MTLManagedObjectAdapterErrorDomain
                                             code:MTLManagedObjectAdapterErrorNoClassFound
                                         userInfo:userInfo];
            }
    ...
    return [adapter modelFromManagedObject:managedObject
                          processedObjects:processedObjects
                                     error:error];
}
                       ...
                       error:(NSError **)error
{
    ...
    NSAssert(entityDescriptionClass != nil,
        @"CoreData.framework must be linked to use MTLManagedObjectAdapter");

    Class fetchRequestClass = NSClassFromString(@"NSFetchRequest");
    NSAssert(fetchRequestClass != nil,
        @"CoreData.framework must be linked to use MTLManagedObjectAdapter");

    // If a uniquing predicate is provided, perform a fetch request to guarantee a unique managed
    // object.
    ...
    if (uniquingPredicate != nil) {
        ...
        managedObject = performInContext(context, ^id {
            ...
            fetchRequest.entity =
                [entityDescriptionClass entityForName:entityName inManagedObjectContext:context];
            ...
            if (results == nil) {
                ...
                if (error != NULL) {
                    NSString *failureReason = [NSString
                        stringWithFormat:
                            NSLocalizedString(
                                @"Failed to fetch a managed object for uniqing predicate \"%@\".",
                                @""),
                        uniquingPredicate];

                    NSDictionary *userInfo = @{
                        NSLocalizedDescriptionKey :
                            NSLocalizedString(@"Could not serialize managed object", @""),
                        NSLocalizedFailureReasonErrorKey : failureReason,
                    };

                    fetchRequestError = [NSError
                        errorWithDomain:MTLManagedObjectAdapterErrorDomain
                                   code:MTLManagedObjectAdapterErrorUniqueFetchRequestFailed
                               userInfo:userInfo];
                }
    ...
    if (managedObject == nil) {
        managedObject = [entityDescriptionClass insertNewObjectForEntityForName:entityName
    ...
    if (managedObject == nil) {
        if (error != NULL) {
            NSString *failureReason = [NSString
                stringWithFormat:
                    NSLocalizedString(
                        @"Failed to initialize a managed object from entity named \"%@\".", @""),
                entityName];
            ...
            *error = [NSError errorWithDomain:MTLManagedObjectAdapterErrorDomain
                                         code:MTLManagedObjectAdapterErrorInitializationFailed
                                     userInfo:userInfo];
        }
    ...
    [dictionaryValue enumerateKeysAndObjectsUsingBlock:^(NSString *propertyKey, id value,
                                                           BOOL *stop) {
        if ([self.propertyKeysToExcludeModel2ManagedObject containsObject:propertyKey]) {
            return;
        }
        NSString *managedObjectKey = [self managedObjectKeyForKey:propertyKey];
        if (managedObjectKey == nil)
        ...
        BOOL (^serializeAttribute)(NSAttributeDescription *)
            = ^(NSAttributeDescription *attributeDescription) {
                // Mark this as being autoreleased, because validateValue may return
                // a new object to be stored in this variable (and we don't want ARC to
                // double-free or leak the old or new values).
                ...
                if (![managedObject validateValue:&transformedValue
                                           forKey:managedObjectKey
                                            error:&tmpError]) {
                    return NO;
                }
        ...
        NSManagedObject * (^objectForRelationshipFromModel)(id) = ^id(id model) {
            if (![model isKindOfClass:MTLModel.class]
                || ![model conformsToProtocol:@protocol(MTLManagedObjectSerializing)]) {
                NSString *failureReason = [NSString
                    stringWithFormat:
                        NSLocalizedString(
                            @"Property of class %@ cannot be encoded into an NSManagedObject.",
                            @""),
                    [model class]];
                ...
                tmpError = [NSError
                    errorWithDomain:MTLManagedObjectAdapterErrorDomain
                               code:MTLManagedObjectAdapterErrorUnsupportedRelationshipClass
                           userInfo:userInfo];

                return nil;
            }

            return [self.class managedObjectFromModel:model
                                 insertingIntoContext:context
                                     processedObjects:processedObjects
                                                error:&tmpError];
        };

        BOOL (^serializeRelationship)(
            NSRelationshipDescription *) = ^(NSRelationshipDescription *relationshipDescription) {
            id relationshipValue = value;
            if (relationshipValue == nil) {
                if (relationshipDescription.isToMany) {
                    if (relationshipDescription.isOrdered) {
                        relationshipValue = [NSOrderedSet orderedSet];
                    }
                    else {
                        relationshipValue = [NSSet set];
                    }
                } else {
                    return YES;
                }
            ...
            if ([relationshipDescription isToMany]) {
                if (![relationshipValue conformsToProtocol:@protocol(NSFastEnumeration)]) {
                    NSString *failureReason = [NSString
                        stringWithFormat:NSLocalizedString(@"Property of class %@ cannot be "
                                             @"encoded into a to-many relationship.",
                                             @""),
                ...
                id originalRelationshipCollection = [managedObject valueForKey:managedObjectKey];
                // 这里假定originalRelationshipCollection中的Object都不是fault状态，测试时，没有出现Fault的情况
                NSMutableSet *originalRelationshipObjectIDs = [NSMutableSet set];
                for (NSManagedObject *object in originalRelationshipCollection) {
                    [originalRelationshipObjectIDs addObject:object.objectID];
                }

                for (MTLModel *model in relationshipValue) {
                    NSManagedObject *nestedObject = objectForRelationshipFromModel(model);
                    if (nestedObject == nil)
                        return NO;
                    if ([originalRelationshipObjectIDs containsObject:nestedObject.objectID]) {
                        [originalRelationshipObjectIDs removeObject:nestedObject.objectID];
                    }
                    [relationshipCollection addObject:nestedObject];
                }

                // 删除无用数据
                for (NSManagedObjectID *managedObjectID in originalRelationshipObjectIDs) {
                    [context deleteObject:[context objectWithID:managedObjectID]];
                }
                [managedObject setValue:relationshipCollection forKey:managedObjectKey];
            }
            else {
                NSManagedObject *nestedObject = objectForRelationshipFromModel(relationshipValue);
                if (nestedObject == nil)
                    return NO;
                NSManagedObject *originalRelationship =
                    [managedObject valueForKey:managedObjectKey];
                // 这里假定originalRelationship不是fault状态，测试时，没有出现Fault的情况
                if (![originalRelationship.objectID isEqual:nestedObject.objectID]) {
                    if (originalRelationship && !originalRelationship.isFault) {
                        [context deleteObject:[context objectWithID:originalRelationship.objectID]];
                    }
                }
        ...
        BOOL (^serializeProperty)(
            ...
            if ([propertyClassName isEqual:@"NSAttributeDescription"]) {
                ...
                tmpError = [NSError
                    errorWithDomain:MTLManagedObjectAdapterErrorDomain
                               code:MTLManagedObjectAdapterErrorUnsupportedManagedObjectPropertyType
                           userInfo:userInfo];
                       ...
                       error:(NSError **)error
{
    ...
    CFMutableDictionaryRef processedObjects
        = CFDictionaryCreateMutable(NULL, 0, &keyCallbacks, &kCFTypeDictionaryValueCallBacks);
    if (processedObjects == NULL)
        return nil;
    ...
    return [self managedObjectFromModel:model
                   insertingIntoContext:context
                       processedObjects:processedObjects
                                  error:error];
}
                       ...
                       error:(NSError **)error
{
    ...
    const void *existingManagedObject
        = CFDictionaryGetValue(processedObjects, (__bridge void *)model);
    ...
    return [adapter managedObjectFromModel:model
                      insertingIntoContext:context
                          processedObjects:processedObjects
                                     error:error];
}
...
- (NSValueTransformer *)entityAttributeTransformerForKey:(NSString *)key
{
    ...
    if ([self.modelClass respondsToSelector:selector]) {
        NSInvocation *invocation = [NSInvocation
            invocationWithMethodSignature:[self.modelClass methodSignatureForSelector:selector]];
...
- (void)mergeValueOfModel:(MTLModel<MTLManagedObjectSerializing> *)model
                   forKey:(NSString *)key
        fromManagedObject:(NSManagedObject *)managedObject
{
...
- (void)mergeValuesOfModel:(MTLModel<MTLManagedObjectSerializing> *)model
  forKeysFromManagedObject:(NSManagedObject *)managedObject
{
    if ([model respondsToSelector:@selector(mergeValuesForKeysFromManagedObject:)]) {
        ...
        [[model.class managedObjectKeysByPropertyKey]
            enumerateKeysAndObjectsUsingBlock:^(NSString *key, NSString *managedObjectKey,
                                                  BOOL *stop) {
...
- (NSPredicate *)uniquingPredicateForModel:(MTLModel<MTLManagedObjectSerializing> *)model
{
    ...
    for (NSString *propertyKey in propertyKeys) {
        ...
        NSPredicate *subpredicate =
            [NSPredicate predicateWithFormat:@"%K == %@", managedObjectKey, transformedValue];
